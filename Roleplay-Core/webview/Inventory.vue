...
const inventory = ref<{ id: string, name: string, quantity: number, icon: string, slot: number, weight: number }[]>([]);
...
const maxWeight = ref(40); // Maximale Traglast
const currentWeight = ref(0); // Aktuelles Gesamtgewicht

function calculateWeight() {
    if (Array.isArray(inventory.value)) {
        currentWeight.value = inventory.value.reduce((total, item) => {
            const weight = item.weight || 0; // Gewicht des Items (Standardwert 0)
            return total + weight * item.quantity;
        }, 0);
    } else {
        console.error('inventory.value ist kein Array!');
    }
}
...
Events.on('updateInventory', (data) => {
    if (Array.isArray(data.inventory)) {
        ...
        maxWeight.value = data.maxWeight || maxWeight.value; // Max Weight nur überschreiben, wenn vorhanden
        calculateWeight(); // Gewicht neu berechnen
    } else {
        console.error('Fehler: Die empfangenen Inventory-Daten sind kein Array!', data);
    }
});
    ...
    <div class="inventory-container">
        <div class="inventory-box">
            ...
            <div class="weight-display">
                <span>Weight: {{ currentWeight }} / {{ maxWeight }} kg</span>
            </div>
            <div class="inventory-grid">
                ...
                >
                    <div 
                        ...
                        <div class="item-icon">
                            <img :src="`/images/${item.id}.png`" alt="item-icon" class="icon-img">
                        </div>
