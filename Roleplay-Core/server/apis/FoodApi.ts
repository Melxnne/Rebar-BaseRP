...
import { foodconfig } from '@Plugins/Roleplay-Core/shared/config.js'; // Config importieren
...
const updateCharacterData = async (player, food: number, water: number) => {
...
export function useFoodAPI() {
    ...
    async function getCharacterFoodAndWater(characterid: string) {
        const characterDocument = getter.byCharacter(characterid);
        if (!characterDocument) {
            alt.log(`Character for ${characterid} not found.` );
            return null;
        }
    ...
    async function addFoodAndWater(characterid: string, foodAmount: number, waterAmount: number): Promise<void> {
        const characterDocument = getter.byCharacter(characterid);
        const document = Rebar.document.character.useCharacter(characterDocument);
        const characterData = document.get();
        ...
        alt.log(`Adding Food: ${foodAmount} and Water: ${waterAmount} to character: ${characterData.name}`);
    ...
    // Funktion zum Entfernen von Food und Water
async function removeFoodAndWater(characterid: string): Promise<void> {
    const characterDocument = getter.byCharacter(characterid);
    const document = Rebar.document.character.useCharacter(characterDocument);
    const characterData = document.get();
    ...
    alt.log(`Removing ${randomFoodDecrease} Food and ${randomWaterDecrease} Water from character: ${characterData.name}`);
    await updateCharacterData(characterDocument, newFood, newWater);
    ...
    async function startFoodWaterTimer() {
        alt.setInterval(() => {
            alt.Player.all.forEach(player => {
                ...
                removeFoodAndWater(characterData._id);
