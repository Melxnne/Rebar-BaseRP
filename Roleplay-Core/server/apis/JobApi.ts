...
async function assignJob(player: alt.Player, characterid: string, jobName: string, grade: number): Promise<void> {
    console.log(`Assigning job ${jobName} with grade ${grade} to ${characterid}`);
    ...
    if (!characterData) {
        alt.log(`Character ${fixName(characterData._id)} not found.`);
        return;
    }
    ...
    try {
        ...
        console.log(`Job ${jobName} with grade ${grade} successfully assigned to ${characterData.name}`);
        alt.log(`Job ${jobName} has been assigned to ${fixName(characterData.name)} with grade ${grade}.`);

        // Aktualisiere das HUD mit den neuen Jobdaten
        const { jobName: updatedJobName, jobGradeName: updatedJobGradeName } = await getJobData(characterData._id);
...
async function removeJob(player: alt.Player, characterid: string, jobName: string): Promise<void> {
    try {
        let characterDocument = getter.byCharacter(characterid);
        const document = Rebar.document.character.useCharacter(characterDocument);
        const characterData = document.get();
        ...
        if (!characterData) {
            alt.log(`Character ${fixName(characterData.name)} not found.`);
            return;
        }
        ...
        if (characterData.jobs) {
            ...
            await document.set("jobs", characterData.jobs);
            await update({ _id: characterData._id, jobs: characterData.jobs }, 'Characters');
            console.log(`Job ${jobName} has been removed from ${characterData.name}.`);
            alt.log(`Job ${jobName} has been removed from ${fixName(characterData.name)}.`);

            // Aktualisiere das HUD, falls der Job entfernt wurde
            const { jobName: updatedJobName, jobGradeName: updatedJobGradeName } = await getJobData(characterid);
            updateHud(player, characterData.cash ?? 0, characterData.bank ?? 0, characterData.black_money, updatedJobName, updatedJobGradeName);
        } else {
            alt.log(`No jobs found for ${fixName(characterData.name)}.`);
        }
    } catch (error) {
        console.error(`Error removing job for character ${CharacterData.name}:`, error);
    }
}
...
async function doesCharacterHaveJob(characterid: string, jobName: string, grade: number): Promise<boolean> {
    try {
        let characterDocument = getter.byCharacter(characterid);
        const document = Rebar.document.character.useCharacter(characterDocument);
        const characterData = document.get();
        ...
        if (!characterData || !characterData.jobs || characterData.jobs.length === 0) {
            alt.log(`Character ${fixName(characterData.name)} has no jobs.`);
            return false;
        }
        ...
        if (job) {
            ...
            alt.log(`Character ${fixName(characterData.name)} does not have the job ${jobName} with grade ${grade}.`);
            return false;
        }
    } catch (error) {
        console.error(`Error checking if character ${CharacterData.name} has job ${jobName} with grade ${grade}:`, error);
        return false;
    }
}

// Funktion zum Abrufen eines Jobs eines Charakters (inklusive Rang)
async function getCharacterJob(characterid: string): Promise<{ name: string; grade: number } | null> {
    try {
        let characterDocument = getter.byCharacter(characterid);
        const document = Rebar.document.character.useCharacter(characterDocument);
        const characterData = document.get();
        ...
        if (!characterData || !characterData.jobs || characterData.jobs.length === 0) {
            alt.log(`No jobs found for character ${fixName(characterData.name)}.`);
            return null;
        }
        ...
        console.error(`Error getting job for character ${CharacterData.name}:`, error);
        return null;
    }
}

// Funktion zum Überprüfen, ob ein Charakter einen Job mit einem bestimmten Rang hat
async function doesCharacterHaveJobGrade(characterid: string, jobName: string, grade: number): Promise<boolean> {
    try {
        let characterDocument = getter.byCharacter(characterid);
        const document = Rebar.document.character.useCharacter(characterDocument);
        const characterData = document.get();
        ...
        console.error(`Error checking job grade for character ${CharacterData.name}:`, error);
        return false;
    }
}

// Funktion zum Abrufen der Jobdaten
async function getJobData(characterid: string): Promise<{ jobName: string, jobGradeName: string }> {
    try {
        // Abrufen der Jobdaten des Spielers
        const jobData = await getCharacterJob(characterid);
        ...
        alt.logError(`Fehler beim Abrufen der Jobdaten für Spieler ${characterid}: ${error.message}`);
        return { 
            jobName: 'Unemployed', 
            jobGradeName: 'N/A' 
